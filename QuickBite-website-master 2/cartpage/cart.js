...
foodItems.forEach((element, index) => {
          ...
          itemDiv.setAttribute("class","cartItem")
          ...
          removeButton.onclick = () => {
              foodItems.splice(index, 1); // Remove item from array
              localStorage.setItem("CartItems", JSON.stringify(foodItems)); // Update local storage
              document.querySelector(".right").innerHTML = ""; // Clear current items
              showItems(resto); // Re-display items
          };
          div3.append(quantity, price, removeButton); // Append button to div3
          itemDiv.append(itemName, div3)
          div2.append(itemDiv)
...
document.getElementById("payment-submit").onsubmit = (event) => {
    event.preventDefault()
    location.href="./thanking.html"
}

document.getElementById("place-order").onclick = (event) => {
    event.preventDefault();
